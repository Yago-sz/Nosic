...
function addCommentToScreen(comment, isAdmin = false) {
    ...
    // Adiciona o comentário no topo se for admin, senão adiciona no final
    if (isAdmin) {
...
// Chame essa função quando for salvar um comentário
formulario.addEventListener('submit', function(e) {
    e.preventDefault();
    const texto = inputText.value.trim();
    if (texto === '') return;

    // Verificar se é admin (exemplo de implementação)
    const isAdmin = false; // Aqui você pode implementar a verificação de admin (login/admin flag)

    // Salvar comentário no Firebase
    const newCommentRef = push(db.ref(`comentarios/${getMusicId()}`));
    set(newCommentRef, {
        texto: texto,
        timestamp: new Date().toISOString(),
        isAdmin: isAdmin  // Aqui define se o comentário é de admin
    }).then(() => {
        addCommentToScreen(texto, isAdmin); // Adiciona o comentário à tela com base no admin
