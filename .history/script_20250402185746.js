...
function alterarImagemCapa(caminhoImagem) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayComments(musicId) {
        commentPost.innerHTML = "<h5>Comentários</h5>"; // Reseta os comentários
    ...
    // Função para salvar novo comentário
    form.addEventListener('submit', (event) => {
        event.preventDefault();

        const musicId = player.src; // Usa a URL da música como identificador único
        ...
        displayComments(musicId); // Atualiza os comentários na tela
    });

    // Atualiza os comentários ao mudar a música
    ...
    // Carrega os comentários da primeira música quando a página inicia
    if (player.src) {
        displayComments(player.src);
    }
});