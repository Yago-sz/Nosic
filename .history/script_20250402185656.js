...
function alterarImagemCapa(caminhoImagem) {
  ...
  ocument.addEventListener("DOMContentLoaded", () => {
    ...
    function displayComments(musicId) {
        commentPost.innerHTML = "<h5>Comentários</h5>"; // Reseta a lista de comentários
    ...
    form.addEventListener('submit', (event) => {
        ...
        const musicId = player.src; // Usa a URL da música como identificador
        const commentText = textComent.value.trim();

        if (commentText === "") return; // Impede comentários vazios
        ...
        displayComments(musicId);
        textComent.value = ""; // Limpa o campo de texto após envio
    });

    // Atualiza os comentários quando a música muda
    player.addEventListener('play', () => {
        displayComments(player.src);
    });

    // Carregar os comentários da primeira música ao iniciar a página
    displayComments(player.src);
});
