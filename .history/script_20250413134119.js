...
function alterarImagemCapa(caminhoImagem) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function addCommentToScreen(commentText, isAdminComment = false, nome = "") {
        ...
        commentContainer.classList.add("comment-container", "p-2", "text-wrap", "text-white");
        ...
        if (isAdminComment) {
            ...
            commentPost.appendChild(commentContainer); // Comentário normal vai pro fim
        }
...
form.addEventListener("submit", function (e) {
    ...
    if (!commentText || !nome) return;

    // Garante que o comentário será tratado como admin apenas se o modo admin estiver ativado
    const comentario = {
        texto: commentText,
        nome: nome,
        timestamp: new Date().toISOString(),
        isAdmin: isAdmin // Isso agora depende do botão "Modo Admin", e não do campo "nome"
    };
