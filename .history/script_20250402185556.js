...
function alterarImagemCapa(caminhoImagem) {
  ...
  form.addEventListener('submit', (event) => {
      ...
      const musicId = player.src; // Identifica a música pelo link da fonte (ou use um ID único)
      ...
      textComent.value = ""; // Limpa a caixa de texto
  });
  ...
  function displayComments(musicId) {
      ...
      if (comments[musicId]) {
          comments[musicId].forEach(comment => {
              ...
              p.classList.add('p-2', 'd-flex', 'text-wrap');
