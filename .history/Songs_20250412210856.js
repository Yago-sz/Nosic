export default [
  {
    id: "musica6",  // ID da música
    src: "./musicas/elvis.mp3",
    name: "Wonder of U",
    cover: "./capas/elvis.jpg",
  },
  {
    id: "musica_1",  // ID da música
    src: "./musicas/feel.mp3",
    name: "d4vd - Feel It",
    cover: "./capas/feel.jpg",
  },
  {
    id: "musica2",  // ID da música
    src: "./musicas/billy.mp3",
    name: "Billy Joel - Vienna",
    cover: "./capas/viena.jpg",
  },
  {
    id: "musica3",  // ID da música
    src: "./musicas/devil.mp3",
    name: "Afterlife - Evanescence",
    cover: "./capas/Devil.jpg",
  },
  {
    id: "musica4",  // ID da música
    src: "./musicas/djavan.mp3",
    name: "Djavan – Oceano",
    cover: "./capas/djavan.jpg",
  },
  {
    id: "musica4",  // ID da música
    src: "./musicas/acdc.mp3",
    name: "AC/DC - Back In Black ",
    cover: "./capas/acdc.jpg",
  },
  {
    id: "musica5",  // ID da música
    src: "./musicas/sun.mp3",
    name: "The Beatles - Here Comes The Sun",
    cover: "./capas/sun.jpg",
  },
  {
    id: "musica6",  // ID da música
    src: "./musicas/soap.mp3",
    name: "Soap",
    cover: "./capas/soap.jpg",
  },
  {
    id: "musica6",  // ID da música
    src: "./musicas/Faroeste Caboclo.mp3",
    name: "Faroeste Caboclo",
    cover: "./capas/faroeste.jpg",
  },
  {
    id: "musica7",  // ID da música
    src: "./musicas/secret.mp3",
    name: "Do You Want To Know A Secret",
    cover: "./capas/secret.jpg",
  },
  {
    id: "musica6",  // ID da música
    src: "./musicas/alicia.mp3",
    name: "Alicia Keys - Fallin",
    cover: "./capas/alicia.jpg",
  },
  {
    id: "musica8",  // ID da música
    src: "./musicas/tim.mp3",
    name: "Timbaland - Apologize ft. OneRepublic",
    cover: "./capas/apolo.jpg",
  },
  {
    id: "musica9",  // ID da música
    src: "./musicas/quis.mp3",
    name: "Maneva - o destino não quis",
    cover: "./capas/quis.jpg",
  },
  {
    id: "musica10",  // ID da música
    src: "./musicas/bah.mp3",
    name: "Labrinth - I'm Tired ",
    cover: "./capas/Céu.jpg",
  },
  {
    id: "musica11",  // ID da música
    src: "./musicas/Black.mp3",
    name: "Black",
    cover: "./capas/black.jpg",
  },
  {
    id: "musica12",  // ID da música
    src: "./musicas/jojo.mp3",
    name: "JoJo's Bizarre Adventure - Opening 2",
    cover: "./capas/joseph1.jpg",
  },
  {
    id: "musica13",  // ID da música
    src: "./musicas/holy.mp3",
    name: "Dio - Holy Diver ",
    cover: "./capas/dio.jpg",
  },
  {
    id: "musica14",  // ID da música
    src: "./musicas/vi.mp3",
    name: "Eu Vi Alguém Mais Belo",
    cover: "./capas/mano.jpg",
  },
  {
    id: "musica15",  // ID da música
    src: "./musicas/lau.mp3",
    name: "Lauryn Hill - Ex-Factor",
    cover: "./capas/lauu.jpg",
  },

];
      



      
