...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function addCommentToScreen(commentText, isAdmin = false, nome = "") {
        const commentContainer = document.createElement("div");
        commentContainer.classList.add("comment-container", "p-2", "text-wrap", "text-white");
    
        let p = document.createElement("p");
    
        // Se o comentário for de admin, ele será destacado
        if (isAdmin) {
            p.innerHTML = `<strong class="text-warning">🛡️ ${nome || "Admin"}</strong><br><span>${commentText}</span>`;
            commentContainer.classList.add("bg-warning-subtle"); // Adiciona destaque
            ...
            commentPost.appendChild(commentContainer); // Comentário normal vai para o final
        }
