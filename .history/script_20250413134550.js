...
function alterarImagemCapa(caminhoImagem) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function addCommentToScreen(commentText, isAdminComment = false, nome = "") {
        const commentContainer = document.createElement("div");
        commentContainer.classList.add("comment-container", "p-2", "text-wrap");
    
        const p = document.createElement("p");
    
        // Se for um comentário marcado como admin e o nome for realmente "Administrador"
        if (isAdminComment && nome === "Administrador") {
            p.innerHTML = `<strong class="text-warning">🛡️ ${nome}</strong><br><span>${commentText}</span>`;
            commentContainer.classList.add("bg-warning-subtle");
            commentPost.insertBefore(commentContainer, commentPost.firstChild); // Comentário do admin vai pro topo
        } else {
            p.innerHTML = `<span>${commentText}</span>`;
            commentContainer.classList.add("bg-white", "text-black");
            commentPost.appendChild(commentContainer); // Comentário comum vai pro final
        }
...
function loadComments() {
    ...
    db.ref(`comentarios/${musicId}`).on('child_added', function(snapshot) {
        ...
        addCommentToScreen(comment.texto, comment.isAdmin, comment.nome);  // Agora passa nome também!
    });
}
...
form.addEventListener("submit", function (e) {
    ...
    if (!commentText || (!nome && !isAdmin)) return;

    // Se for admin, define nome fixo
    if (isAdmin) {
        nome = "Administrador";
    }
    ...
    db.ref("comentarios/" + musicId).push(comentario);
