...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function addCommentToScreen(commentText, timestamp, commentKey, nome = null, isAdminComment = false) {
        ...
        // Adiciona destaque se for de admin
        if (isAdminComment) {
            commentContainer.classList.add("bg-warning-subtle", "border", "border-warning"); // destaque visual
        } else {
        ...
        // Sempre mostra o nome do admin
        if (isAdminComment && nome) {
            innerHtml += `<strong class="text-warning">🛡️ ${nome} (admin)</strong><br>`;
        } else if (nome) {
            innerHtml += `<strong>👤 ${nome}</strong><br>`;
        }
        ...
        // Botão de deletar só aparece se estiver no modo admin
        if (isAdmin) {
