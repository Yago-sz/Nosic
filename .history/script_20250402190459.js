...
function alterarImagemCapa(caminhoImagem) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function getMusicId() {
        console.log("Obtendo ID da música...");
        return player ? player.src || "default" : "default";
    }

    // Função para carregar e exibir os comentários na tela
    function displayComments() {
        console.log("Exibindo comentários...");
        commentPost.innerHTML = "<h5>Comentários</h5>";

        let comments = JSON.parse(localStorage.getItem('comments')) || {};
        let musicId = getMusicId();
        console.log("ID da música atual:", musicId);
    ...
    // Evento para adicionar comentários ao clicar no botão
    form.addEventListener('submit', (event) => {
        ...
        console.log("Comentário digitado:", commentText);

        if (commentText === "") {
            console.log("Comentário vazio, não será salvo.");
            return;
        }
