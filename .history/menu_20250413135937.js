...
function addCommentToScreen(commentText, timestamp, commentKey, nome = null, isAdminComment = false) {
    ...
    // Adiciona destaque para comentários do admin
    if (isAdminComment) {
        commentContainer.classList.add("bg-warning-subtle", "border", "border-warning"); // Destaque para admin
    } else {
        commentContainer.classList.add("bg-white"); // Comentários normais
    }
    ...
    // Botão de deletar (visível apenas para o admin logado)
    if (isAdmin) {
    ...
    if (isAdminComment) {
        ...
        commentPost.appendChild(commentContainer); // Coloca no final para comentários normais
    }
}
